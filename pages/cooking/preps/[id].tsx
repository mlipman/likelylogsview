...
export default function PrepDetail() {
  ...
  useEffect(() => {
    ...
    const fetchPrep = async () => {
      try {
        ...
        const foundPrep = preps.find((p: Prep) => p.id === parseInt(id as string));
        
        if (foundPrep) {
          setPrep(foundPrep);
          setFormData({
            occurred_at: foundPrep.occurred_at ? foundPrep.occurred_at.split('T')[0] : "",
            plan_md: foundPrep.plan_md || "",
            outcome_md: foundPrep.outcome_md || "",
            result_pic_ids: foundPrep.result_pic_ids || "",
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedPrep = await response.json();
        setPrep({...prep!, ...updatedPrep});
        setIsEditing(false);
      } else {
        console.error("Failed to update prep");
      }
    } catch (error) {
  ...
  const handleCancel = () => {
    if (prep) {
      setFormData({
        occurred_at: prep.occurred_at ? prep.occurred_at.split('T')[0] : "",
        plan_md: prep.plan_md || "",
        outcome_md: prep.outcome_md || "",
        result_pic_ids: prep.result_pic_ids || "",
      });
    }
    setIsEditing(false);
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading prep...</p>
        </div>
      </div>
    );
  }
  ...
  if (!prep) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Prep not found</p>
          <Link
            href="/cooking/preps"
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              {isEditing ? "Edit Prep" : `Prep Session`}
            </h1>
            <p className="text-gray-600">
              {prep.week.year} - Week {prep.week.week}
              {prep.project && ` • ${prep.project.title}`}
            </p>
          </div>
          {!isEditing && (
            <button
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={6}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500"
                    placeholder="How did the prep go? What did you produce? Storage notes, yield, quality, what worked well, what to improve next time..."
                  />
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Enter comma-separated Cloudinary image IDs for photos of your prep results
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="text-gray-900">{prep.week.year} - Week {prep.week.week}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-1">Project</h3>
                  <p className="text-gray-900">{prep.project ? prep.project.title : "No project"}</p>
                </div>
                <div>
              ...
              {prep.result_pic_ids && (
                <div>
                  ...
                  <p className="text-sm text-gray-700">{prep.result_pic_ids}</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  {new Date(prep.updated_at).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8">
          <Link href="/cooking/preps" className="text-orange-600 hover:text-orange-800">
            ← Back to Preps
          </Link>
        </div>
      </div>
